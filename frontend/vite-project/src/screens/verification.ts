...
export function initVerificationPortal(containerId: string): void {
  ...
  if (sampleBtn && input) {
    sampleBtn.addEventListener('click', async () => {
      ...
      try {
        const pack = await etpClient.getSampleProofPack();
        input.value = JSON.stringify(pack, null, 2);
      } catch (err: any) {
        alert('Failed to load sample proof pack from server: ' + (err.message || err));
      } finally {
        sampleBtn.textContent = 'Load Sample Proof Pack';
      }
    });
  }
